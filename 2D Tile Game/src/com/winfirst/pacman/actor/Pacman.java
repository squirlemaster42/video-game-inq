...
public class Pacman extends PacmanActor {
    ...
    public void init() {
        ...
        for (int d=0; d<4; d++) {
            for (int i=0; i<4; i++) {
                pacmanFrameNames[i + 4 * d] = "/pacman_" + d + "_" + i + ".png";
            }
        }
        for (int i=0; i<14; i++) {
            pacmanFrameNames[16 + i] = "/pacman_died_" + i + ".png";
        }
    ...
    public void updatePlaying() {
        ...
        if (super.game.handler.getKeyManager().left) {
            desiredDirection = 2;
        }
        else if (super.game.handler.getKeyManager().right) {
            desiredDirection = 0;
        }
        else if (super.game.handler.getKeyManager().up) {
            desiredDirection = 3;
        }
        else if (super.game.handler.getKeyManager().down) {
            desiredDirection = 1;
        }
    ...
    public void stateChanged() {
        ...
        else if (game.getState() == PacmanGame.State.PLAYING) {
            instructionPointer = 0;
        }
